...
fn positive_usize_rejects_zero() {
    assert!(super::positive_usize(Some("0".to_owned())).is_none());
    assert!(super::positive_usize(Some("-1".to_owned())).is_none());
}

#[test]
fn positive_usize_accepts_valid_values() {
    assert_eq!(super::positive_usize(Some("1".to_owned())), Some(1));
    assert_eq!(super::positive_usize(Some("100".to_owned())), Some(100));
}

#[test]
fn positive_u64_rejects_zero() {
    assert!(super::positive_u64(Some("0".to_owned())).is_none());
}

#[test]
fn positive_u64_accepts_valid_values() {
    assert_eq!(super::positive_u64(Some("1".to_owned())), Some(1));
    assert_eq!(super::positive_u64(Some("120".to_owned())), Some(120));
}
...
fn setting_at_returns_none_for_missing_key() {
    ...
    assert!(super::setting_at(&path, "missing_key").is_none());
}
...
fn setting_at_returns_none_for_missing_file() {
    let result = super::setting_at(std::path::Path::new("/nonexistent/settings.json"), "key");
    assert!(result.is_none());
}

#[test]
fn subscription_limits_new_validates_max_processes() {
    assert!(super::SubscriptionLimits::new(0, 60).is_err());
    assert!(super::SubscriptionLimits::new(1, 60).is_ok());
}

#[test]
fn subscription_limits_new_validates_timeout() {
    assert!(super::SubscriptionLimits::new(5, 0).is_err());
    assert!(super::SubscriptionLimits::new(5, 60).is_ok());
}
