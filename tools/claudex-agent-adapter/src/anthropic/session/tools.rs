...
fn parallel_scheduler_instructions(request: &MessagesRequest) -> String {
    ...
    let mut lines = vec![
        format!(
            "Runtime parallel policy: dynamically size substantive work to at least {} ordinary workers across at least {} model families, matching independent scopes when higher, and never more than {}. Do not start with one Explore and do not blindly launch the concurrent cap. Only an atomic lookup/command stays at one worker. Maintain at least {} active lanes during a completion or cadence rebalance.",
            config.min_parallel_workers, config.min_model_families, config.max_parallel_workers, config.active_floor
        ),
        format!(
