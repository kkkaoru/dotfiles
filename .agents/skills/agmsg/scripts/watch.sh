...
# claudex: automatic agmsg Monitor is opt-in for the interactive parent.
if [ "${CLAUDEX_ACTIVE:-}" = 1 ] \
  && [ "${CLAUDEX_AGMSG_AUTO_MONITOR:-}" != 1 ] \
  && [ "${CLAUDEX_AGMSG_EXPLICIT:-}" != 1 ]; then
  exit 0
fi
# claudex: provider/noninteractive child watchers are disabled.
if [ "${CLAUDEX_NONINTERACTIVE_CHILD:-}" = 1 ] \
  || [ "${CLAUDEX_PROVIDER_ACP:-}" = 1 ] \
  || [ "${CLAUDEX_GROK_ACP:-}" = 1 ]; then
  exit 0
fi
# claudex: resumed claudex sessions do not run agmsg watchers.
...
# claudex: serialize same-session watcher claims.
# claudex: watcher claim schema v2.
