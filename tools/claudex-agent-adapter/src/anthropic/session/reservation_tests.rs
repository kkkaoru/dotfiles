...
#[test]
fn claude_session_ids_match_compares_optional_pairs() {
    assert!(claude_session_ids_match(Some("sess-1"), Some("sess-1")));
    assert!(!claude_session_ids_match(Some("sess-1"), Some("sess-2")));
    assert!(claude_session_ids_match(None, None));
    assert!(!claude_session_ids_match(Some("sess-1"), None));
    assert!(!claude_session_ids_match(None, Some("sess-1")));
}
