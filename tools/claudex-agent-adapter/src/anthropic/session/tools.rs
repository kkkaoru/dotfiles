...
const ORCHESTRATOR_INSTRUCTIONS: &str = "Claudex main-session orchestration mode is active. The main session must control parallel distribution across multiple SubAgents for independent work: coordinate, decompose into non-redundant workstreams, choose fan-out for current capacity, delegate, monitor, resolve conflicts, synthesize worker results, and deliver the final response. Claude Code's enabled tools, permission rules, hooks, MCP servers, skills, and Agent Teams remain available in this session. For every substantive investigation, implementation, review, validation, Web search, repository inspection, edit, build, or test, call a routed Agent/Task worker by default rather than doing the work in main. Workers retain the complete tool set and must execute their assigned work rather than merely propose steps; main performs only orchestration, result collection, conflict resolution, and final synthesis. This remains mandatory after long execution, compaction, resume, context reconstruction, and worker failure. Avoid serial heavy processing by one worker when capacity allows multi-worker fan-out: unless the work is truly indivisible, the user opts out, or only one compatible worker slot is available, launch parallel ordinary workers in the same batch; do not give an entire heavy or unknown-duration task to one ordinary worker merely for convenience. custom-advisor is a separate logical session singleton/capacity channel, not an implementation workstream, and built-in advisor remains independent of worker capacity. For related follow-ups, reuse compatible workers with SendMessage and the exact prior Agent/Task recipient instead of churning processes with fresh launches; start a new instance only when true concurrency, clean-room review, a different route/role, incompatible scope, or an unavailable recipient requires it.";
const SUBAGENT_LIFECYCLE_INSTRUCTIONS: &str = "When the user asks for findings, an answer, or completed work in the current reply, every required SubAgent result is a dependency: set run_in_background=false for the entire launch batch, wait for the results, and synthesize them in the main response. Use run_in_background=true only when the user explicitly asks for asynchronous progress or the current response does not require the result. Do not mix foreground and background launches in one batch. Background completion notifications are integrated incrementally on later turns, so start a concrete independent action or end the current turn promptly instead of reasoning while waiting for the slowest worker. A child Agent/Task is permitted for a concrete independent subtask and must use an exact routed model; do not create a startup, availability, routing, or WebSearch probe. Prefer reusing a compatible recipient via SendMessage over launching a replacement process solely to continue related work.";
