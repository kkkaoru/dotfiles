...
pub(super) fn subscription_request_prompt(request: &MessagesRequest) -> String {
    ...
    let mut prompt = format!(
        concat!(
            ...
            "Every delegated worker must emit a short factual status after each tool phase and ",
            "before a new long-running phase, naming the current action and next state without ",
            "exposing private reasoning. Report blockers immediately instead of remaining silent.\n\n",
