...
pub(super) fn subscription_request_prompt(request: &MessagesRequest) -> String {
    format!(
        "Act as the requested Claude Code model. Follow the system instructions and complete the conversation below. Use only the enabled tools when needed. When delegation is requested and selected_workers are present, invoke the selected Agent or Task directly as the first tool call; do not perform task-list bookkeeping first. Apply current selected_workers routing to every Agent/Task launch, including nested launches from an existing worker: choose the selected claudex worker agent and pass its exact model and effort. Never default a nested launch to generic claude or blindly inherit its parent provider when current routing selects another route. Start as many instances as useful for real parallelism. For a related follow-up, use SendMessage with the exact compatible recipient specified by the prior Agent/Task result (agent ID or teammate name as applicable), sending the smallest sufficient self-contained delta including unseen evidence. Before shutdown or replacement, weigh likely follow-ups and potential prompt-prefix/cache reuse against slot/resource pressure and stale context; neither creation nor termination is categorically forbidden. Treat current routing context as authoritative over stale model-policy memory. When a Task or Agent tool schema lacks claudex_model or claudex_effort, put each routed value at the start of its prompt as an exact `claudex_model: <model>` or `claudex_effort: <effort>` line. Never put a gpt or grok ID in the native model field.\n\nSystem:\n{}\n\nMessages:\n{}",
