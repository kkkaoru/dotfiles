...
pub(super) fn subscription_request_prompt(request: &MessagesRequest) -> String {
    format!(
        concat!(
            ...
            "parallelism. When the main session needs their results now, launch independent workers ",
            "together as foreground calls; use background only when useful independent work can ",
            "continue or the task must outlive the turn. For a related follow-up, use SendMessage ",
            ...
            "including unseen evidence. Reuse the first compatible session advisor for related ",
