...
# ---------------------------------------------------------------------------
# Spotlight: キャッシュやビルド生成物を索引対象から除外
#   Spotlight 検索そのものは有効なままにする。
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ctx: 履歴検索ストレージを4 GiB以内に維持
# ---------------------------------------------------------------------------
if [[ -f "${SCRIPT_DIR}/ctx-size-guard.sh" ]]; then
  chmod +x "${SCRIPT_DIR}/ctx-size-guard.sh"
  "${SCRIPT_DIR}/ctx-size-guard.sh" --install
else
  echo "--> ctxサイズガードをスキップ (ctx-size-guard.sh が見つかりません)"
fi
