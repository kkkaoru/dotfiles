...
Do not confuse this Flash route (`opencode-go/deepseek-v4-flash` / `claudex-deepseek-flash`) with
the Pro route (`opencode-go/deepseek-v4-pro` / `claudex-deepseek-pro`). Always pass the Flash
model id when launching or nesting this worker. If Flash is listed in
`disabled_subagent_models`, do not auto-select this worker.
