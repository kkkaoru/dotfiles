...
| --- | --- | --- | --- | --- |
| Orchestrator | `claudex-orchestrator` | `gpt-5.6-sol` | Claude Code設定 | 引数なしの通常起動 |
...
1. 引数なしの `claudex` は `mainProvider` のモデルで
   `claudex-orchestrator` を起動します。
...
指定値は `modelPrefixes` と照合されます。設定にないprefixのモデルは起動時に拒否されます。
