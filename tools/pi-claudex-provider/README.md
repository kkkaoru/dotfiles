...
| `thinking_start` / `thinking_delta` / `thinking_end` | `index`; `delta` or final `content`                       | Preserve order and block index. The protocol v1 lifecycle does not carry opaque thinking signatures, so retain them from the authoritative terminal message and do not replace them with synthetic provider signatures for replay. | Thinking disappears, or a follow-up fails when a provider rejects its signature.    |
