...
async fn inspect_hot_busy_state(client: &reqwest::Client, reusable: &mut ServiceConfig) {
...
async fn inspects_start_reuse_and_replacement_service_states() {
    let client = reqwest::Client::new();
    inspect_absent_and_silent_states(&client).await;
    let mut reusable = inspect_reusable_and_attached_states(&client).await;
...
fn matching_build_dummy_script(config: &ServiceConfig) -> String {
    ...
    format!(
        ...
        search_worker_routes =
            serde_json::to_string(&search_worker_routes).expect("search worker routes json"),
...
fn write_matching_build_dummy(root: &Path, config: &ServiceConfig) -> PathBuf {
    ...
    let script = matching_build_dummy_script(config);
...
async fn ensure_replace_mentions_an_attached_launch_tui() {
    ...
    let dummy = write_matching_build_dummy(root.path(), &cfg);
    cfg.executable = dummy.clone();
    let mut launch = Command::new(&launch_bin)
        .args([
            "launch",
            "--listen",
